...
<!-- This code depends in the DITA Community common.xslt library dita-support-lib.xsl.
  ...
  <xsl:template 
    match="*[df:class(., 'd4p-variables-d/d4p-variableref_text')] |
           ...
           *[df:class(., 'dc-variables-d/dc-variableref_text')] |
    ...
    <xsl:if test="$doDebug">
      ...
      <xsl:message> + [DEBUG] dc-variableref: topicref=      
        <xsl:sequence select="df:reportTopicref($topicref)"/>      
      </xsl:message>    
    </xsl:if>
      ...
      <xsl:choose>
        <xsl:when test="count($localVarDef) = 0 and count($topicref) > 0">
          ...
          <xsl:variable name="metadataAncestry" as="element()*"
            select="reverse($topicref/ancestor::*[df:class(., 'map/topicref') or
                               df:class(., 'map/map')]/*[df:class(., 'map/topicmeta')])"
          />
    ...
    <xsl:choose>
      <xsl:when test="count($varDef) = 1">
        <span class="{if (@outputclass != '') then @outputclass else 'dc-variableref'}"
        >
          <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="localFallbackDefs" as="element()*"
          select="
          ancestor::*/*[df:class(., 'dc-variables-d/dc-variable-definitions') or df:class(., 'd4p-variables-d/d4p-variable-definitions')]//*[df:class(., 'dc-variables-d/dc-variable-definition') or df:class(., 'd4p-variables-d/d4p-variable-definition')][@name = $variableName] |
          ancestor::*[df:class(., 'topic/topic')]/*[df:class(., 'topic/prolog')]//*[df:class(., 'dc-variables-d/dc-variable-definition-fallback') or df:class(., 'd4p-variables-d/d4p-variable-definition-fallback')][@name = $variableName]"
        />
        <xsl:if test="$doDebug">
